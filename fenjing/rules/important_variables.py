...
# These joke rules are highly environment-dependent and can produce flaky
# payloads on real targets such as ctfshow web370, so keep them disabled by
# default and prefer deterministic chains first.
is_brainrot_enabled = False
...
@expression_gen
def gen_module_os_lipsum(context):
    return [
        (
            CHAINED_ATTRIBUTE_ITEM,
            (JINJA_CONTEXT_VAR, "lipsum"),
