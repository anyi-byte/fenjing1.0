...
@expression_gen
def gen_string_chunked(context: dict, value: str):
    if len(value) < 24:
        return [(UNSATISFIED,)]
    chunk_size = 12
    parts = [value[i : i + chunk_size] for i in range(0, len(value), chunk_size)]
    if len(parts) <= 1:
        return [(UNSATISFIED,)]
    target_list = (
        [(LITERAL, "(")]
        + join_target((LITERAL, ","), [(STRING, part) for part in parts])
        + [(LITERAL, ")|join")]
    )
    return [(EXPRESSION, precedence["plain_filter"], target_list)]
