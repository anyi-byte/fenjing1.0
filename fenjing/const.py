...
SET_STMT_PATTERNS = [
    # (fill_pattern, test_pattern, expr_pattern, precedence_name, requires_flask)
    (
    ...
    ("{%set(NAME)=EXPR%}", "{%set(a)=%}", "NAME", "literal", False),
    (
        "{{config.update(NAME=EXPR)}}",
        "{{config.update(=)}}",
        "config.NAME",
        "attribute",
        True,  # config only exists in Flask template context
    ),
]
