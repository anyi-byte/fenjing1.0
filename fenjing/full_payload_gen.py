...
class FullPayloadGen:
    ...
    def try_add_context_var(
        ...
        is_flask = self.options.environment == TemplateEnvironment.FLASK
        for (
            fill_pattern,
            test_pattern,
            test_expr_pattern,
    ...
    def prepare_extra_context_vars(self, append_targets: List[str]):
        ...
        is_flask = self.options.environment == TemplateEnvironment.FLASK
        if not any(
            self.waf_func(test_pattern)
            for _, test_pattern, _, _, requires_flask in SET_STMT_PATTERNS
            if not requires_flask or is_flask
        ):
            logger.info(
