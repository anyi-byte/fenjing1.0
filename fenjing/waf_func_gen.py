...
class WafFuncGen:
    ...
    def normal_page_hash(self) -> List[int]:
        """获取普通无害输入对应的页面哈希，用于无回显场景基线判定。"""
        hashes: List[int] = []
        keywords = [
            "".join(random.choices(string.ascii_lowercase, k=6))
            for _ in range(8)
        ]
        with pbar_manager.pbar(keywords, "normal_page_hash") as keywords:
        ...
        if not hashes:
            return []
        common_hashes = [k for k, v in Counter(hashes).items() if v >= 2]
        return common_hashes if common_hashes else list(set(hashes))
    ...
    def generate(self) -> WafFunc:
        ...
        normal_hashes = self.normal_page_hash() if self.options.no_echo.enabled else []
        unique_waf_hashes = [h for h in waf_hashes if h not in normal_hashes]
        waf_keywords = (
            []
            if self.options.detect_waf_keywords == DetectWafKeywords.NONE
            else self.waf_keywords(unique_waf_hashes)
        )
        replaced_keyword = self.replaced_keyword()
        long_param_hashes = self.long_param_hash()
        long_param_hashes = [h for h in long_param_hashes if h not in unique_waf_hashes]
        if (
